...
class ComputeSELDResults(object):
    ...
    def get_SELD_Results(self, pred_files_path, is_jackknife=False):
        ...
        pred_labels_dict = {}
        if self.segment_level:
            eval = evaluation_metrics.SELDMetricsSegmentLevel(nb_classes=self._feat_cls.get_nb_classes(),
                                                       doa_threshold=self._doa_thresh, average=self._average)
        else:
            eval = evaluation_metrics.SELDMetrics(nb_classes=self._feat_cls.get_nb_classes(),
